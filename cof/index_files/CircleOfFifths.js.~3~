...
document.addEventListener("keypress", function onPress(event) {
    if (event.key === "j") {
	let keys = Object.keys(g_dTonicInfo);
	g_sSelTonic = keys[(keys.indexOf(g_sSelTonic)+1) % keys.length];
	update();
    }
});
...
document.addEventListener("keypress", function onPress(event) {
	...
	let keys = Object.keys(g_dTonicInfo);
	let prev = 
	g_sSelTonic = keys[(keys.indexOf(g_sSelTonic) + (keys.length - 1)) % keys.length]
	update();
    }
});
...
document.addEventListener("keypress", function onPress(event) {
    if (event.key === "h") {
	let keys = Object.keys(g_dModeInfo);
	g_sSelMode = keys[(keys.indexOf(g_sSelMode)+1) % keys.length];
	update();
    }
});
...
document.addEventListener("keypress", function onPress(event) {
    if (event.key === "l") {
	let keys = Object.keys(g_dModeInfo);
	g_sSelMode = keys[(keys.indexOf(g_sSelMode) + (keys.length - 1)) % keys.length]
	update();
    }
});
